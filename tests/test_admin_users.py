...
def test_admin_create_user_generates_password_hash_and_area_links() -> None:
    ...
    assert created["can_use_issue_reports"] is True
    assert verify_password("senha-segura", user_row["password_hash"]) is True
    assert user_row["can_use_issue_reports"] is True
    assert [row["area_id"] for row in client.tables["user_area_access"]] == ["area-A", "area-B"]


def test_admin_create_user_accepts_plain_login_without_email_format() -> None:
    client = FakeClient()
    repository = AdminUserRepository(client=client)  # type: ignore[arg-type]

    created = repository.create_user(
        AdminUserPayload(
            email=" solicitante.issue ",
            name="Solicitante Issue",
            role="gestor_area",
            password="senha-segura",
            can_use_issue_reports=True,
            area_ids=[],
    ...
    assert created["email"] == "solicitante.issue"
    assert created["area_ids"] == []
    assert created["can_use_issue_reports"] is True


def test_admin_create_view_only_executive_without_area() -> None:
    client = FakeClient()
    repository = AdminUserRepository(client=client)  # type: ignore[arg-type]
    ...
    created = repository.create_user(
        AdminUserPayload(
            email="viewer",
            name="Viewer",
            role="executivo_visualizacao",
            password="senha-segura",
            area_ids=[],
        )
    )

    assert created["role"] == "executivo_visualizacao"
    assert created["area_id"] is None
    assert created["area_ids"] == []
...
def test_admin_update_user_replaces_area_links() -> None:
    ...
    assert updated["can_use_issue_reports"] is True
