...
class FakeIndicatorRepository:
    def __init__(self, indicator: Indicator, existing_names: set[str] | None = None) -> None:
        ...
        self.last_target_upsert: IndicatorMonthTarget | None = None
        self.deleted_targets: list[tuple[str, int, int]] = []
        self.deleted_projections: list[tuple[str, int, int]] = []
        self.not_applicable_items: list[IndicatorMonthNotApplicable] = []
    ...
    def list_weekly_values(
        ...
        return [
            value
            for value in self.weekly_values
            if value.indicator_id in indicator_ids
            and value.year == year
    ...
    def list_month_targets(self, indicator_ids: list[str], year: int) -> list:
        return [
            target
            for target in self.month_targets
            if target.indicator_id in indicator_ids and target.year == year
        ]

    def list_month_projections(self, indicator_ids: list[str], year: int) -> list:
        return [
            projection
            for projection in self.month_projections
            if projection.indicator_id in indicator_ids and projection.year == year
        ]
    ...
    def delete_month_target(self, indicator_id: str, year: int, month: int) -> None:
        self.deleted_targets.append((indicator_id, year, month))

    def delete_month_projection(self, indicator_id: str, year: int, month: int) -> None:
        self.deleted_projections.append((indicator_id, year, month))
    ...
    def set_month_not_applicable(
        ...
        self.not_applicable_items = [
            item
            for item in self.not_applicable_items
            if not (item.indicator_id == indicator_id and item.year == year and item.month == month)
        ]
        if is_not_applicable:
            self.not_applicable_items.append(
                IndicatorMonthNotApplicable(
                    indicator_id=indicator_id,
                    year=year,
                    month=month,
...
def test_view_only_executive_lists_all_indicators() -> None:
    indicator = Indicator(
        id="ind-1",
        area_id="area-B",
        area_name="Area B",
    ...
    rows = use_case.execute(user=build_user(role="executivo_visualizacao"), year=2026)
...
def test_view_only_executive_cannot_create_indicator() -> None:
    indicator = Indicator(
        id="ind-1",
        area_id="area-A",
        area_name="Area A",
    ...
    with pytest.raises(AuthorizationError):
        use_case.execute(
            user=build_user(role="executivo_visualizacao"),
            area_id="area-A",
            name="Novo indicador",
            description="",
            aggregation_type="sum",
...
def test_create_indicator_accepts_maturity_boundaries() -> None:
    indicator = Indicator(
        id="ind-1",
        area_id="area-A",
        area_name="Area A",
    ...
    created = use_case.execute(
        ...
        area_id="area-A",
        name="Indicador Maduro",
        description="",
        aggregation_type="sum",
        unit_id="unit-percent",
        maturity_level=Decimal("100"),
    )

    assert created.maturity_level == Decimal("100")
    assert repository.created_indicators[0].maturity_level == Decimal("100")


def test_update_indicator_rejects_maturity_outside_range() -> None:
    indicator = Indicator(
        id="ind-1",
        area_id="area-A",
        area_name="Area A",
    ...
    repository = FakeIndicatorRepository(indicator=indicator)
    use_case = UpdateIndicator(indicator_repository=repository)
...
def test_upsert_month_projection_empty_value_deletes_saved_projection() -> None:
    indicator = Indicator(
        id="ind-1",
        area_id="area-A",
        area_name="Area A",
    ...
    saved = use_case.execute(
        user=build_user(role="executivo", can_edit_projected_value=True),
        indicator_id="ind-1",
        year=2026,
        month=6,
    ...
    assert repository.deleted_projections == [("ind-1", 2026, 6)]


def test_upsert_month_target_empty_value_deletes_saved_target() -> None:
    indicator = Indicator(
        id="ind-1",
        area_id="area-A",
        area_name="Area A",
    ...
    repository = FakeIndicatorRepository(indicator=indicator)
    use_case = UpsertIndicatorMonthTarget(indicator_repository=repository)
    ...
    assert saved is None
    assert repository.deleted_targets == [("ind-1", 2026, 6)]


def test_month_not_applicable_hides_monthly_value_and_keeps_planning_values() -> None:
    indicator = Indicator(
        id="ind-1",
        area_id="area-A",
        area_name="Area A",
    ...
    repository.month_projections = [
        IndicatorMonthProjection(
            ...
            year=2026,
            month=6,
            projected_value=Decimal("80"),
            created_by="exec",
            updated_by="exec",
        )
    ]
    repository.not_applicable_items = [
        IndicatorMonthNotApplicable(
            indicator_id="ind-1",
            year=2026,
            month=6,
    ...
    assert rows[0].not_applicable[6] is True
    assert rows[0].monthly_values[6] is None
...
def test_manager_can_toggle_month_not_applicable_for_own_area() -> None:
    ...
    repository = FakeIndicatorRepository(indicator=indicator)
    use_case = SetIndicatorMonthNotApplicable(indicator_repository=repository)
    manager = build_user(role="gestor_area", area_id="area-A")
    ...
    assert len(repository.not_applicable_items) == 1
    ...
    assert repository.not_applicable_items == []


def test_manager_cannot_toggle_month_not_applicable_for_other_area() -> None:
    indicator = Indicator(
        id="ind-1",
        area_id="area-A",
        area_name="Area A",
    ...
    with pytest.raises(AuthorizationError):
        use_case.execute(
            user=build_user(role="gestor_area", area_id="area-B"),
            indicator_id="ind-1",
            year=2026,
            month=6,
            is_not_applicable=True,
