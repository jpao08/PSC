...
function renderUsers() {
  ...
  thead.innerHTML = "<tr><th>Nome</th><th>Usuario/email</th><th>Perfil</th><th>Areas</th><th>Permissoes</th><th>Status</th><th>Acoes</th></tr>";
  ...
  state.users.forEach((user) => {
    ...
    tr.innerHTML = `
      ...
      <td>${user.can_use_issue_reports ? "Issue Reports" : "-"}</td>
  ...
  if (state.users.length === 0) {
    const tr = document.createElement("tr");
    tr.innerHTML = '<td colspan="7" class="muted">Nenhum usuario encontrado.</td>';
    tbody.appendChild(tr);
  }
...
function clearForm() {
  ...
  document.getElementById("can-use-issue-reports").checked = false;
...
function fillForm(user) {
  ...
  document.getElementById("can-use-issue-reports").checked = !!user.can_use_issue_reports;
...
function buildPayload() {
  return {
    ...
    can_use_issue_reports: document.getElementById("can-use-issue-reports").checked,
