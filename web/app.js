...
const issuePriorityPanel = document.getElementById("issue-priority-panel");
const issuePriorityTitle = document.getElementById("issue-priority-title");
const issuePriorityForm = document.getElementById("issue-priority-form");
const issuePriorityId = document.getElementById("issue-priority-id");
const issueExecutiveGravity = document.getElementById("issue-executive-gravity");
const issueExecutiveUrgency = document.getElementById("issue-executive-urgency");
const issueExecutiveTendency = document.getElementById("issue-executive-tendency");
const issueExecutiveScore = document.getElementById("issue-executive-score");
const issueDateFrom = document.getElementById("issue-date-from");
const issueDateTo = document.getElementById("issue-date-to");
const issueRequesterFilter = document.getElementById("issue-requester-filter");
const issueAreaFilter = document.getElementById("issue-area-filter");
const issueTagFilter = document.getElementById("issue-tag-filter");
const issueTitleFilter = document.getElementById("issue-title-filter");
const issueTitleSuggestions = document.getElementById("issue-title-suggestions");
const issueSort = document.getElementById("issue-sort");
const manageTagsBtn = document.getElementById("manage-tags-btn");
const issueTagsPanel = document.getElementById("issue-tags-panel");
const issueTagForm = document.getElementById("issue-tag-form");
const issueTagId = document.getElementById("issue-tag-id");
const issueTagName = document.getElementById("issue-tag-name");
const issueTagColor = document.getElementById("issue-tag-color");
const issueTagSubmit = document.getElementById("issue-tag-submit");
...
function buildMonthCellContent(monthItem) {
  ...
  valueNode.textContent = monthItem && monthItem.not_applicable
    ? "N/A"
    : formatNumber(monthItem ? monthItem.value : null);
...
function applyLoggedInView() {
  ...
  if (canUseGlobalIndicatorFilters()) {
...
function canUseIssueReports() {
  return !!(
    state.user
    && (state.user.role === "executivo"
      || state.user.can_use_issue_reports)
  );
}
...
function canUseGlobalIndicatorFilters() {
  return !!(
    state.user
    && (state.user.role === "executivo" || state.user.role === "executivo_visualizacao")
  );
}
...
function issueAreaLabel(issue) {
  if (issue.is_other_area) {
    return "Outras";
  }
  return issue.area_name || issue.area_id || "-";
}

function issueCreatedDate(issue) {
  return issue.created_at ? new Date(issue.created_at) : new Date(0);
}
...
async function loadIssueReports() {
  ...
  populateIssueAreaOptions();
  state.issueTags = await api("/api/issue-tags");
  state.issueReports = await api("/api/issue-reports");
  manageTagsBtn.classList.toggle("hidden", !(state.user && state.user.role === "executivo"));
  renderIssueTagsPanel();
  populateIssueFilters();
  renderIssueReports();
}

function populateIssueAreaOptions() {
  issueAreaSelect.innerHTML = "";
  state.areas.forEach((area) => {
    const option = document.createElement("option");
    option.value = area.id;
    option.textContent = area.name;
    issueAreaSelect.appendChild(option);
  });
...
function resetIssueTagForm() {
  issueTagForm.reset();
  issueTagId.value = "";
  issueTagColor.value = DEFAULT_AREA_COLOR;
  issueTagSubmit.textContent = "Salvar Tag";
}

function renderIssueTagsPanel() {
  issueTagsList.innerHTML = "";
  if (!state.issueTags || state.issueTags.length === 0) {
    const empty = document.createElement("p");
    empty.className = "muted";
    empty.textContent = "Nenhuma tag cadastrada.";
    issueTagsList.appendChild(empty);
    return;
  }

  state.issueTags.forEach((tag) => {
    const row = document.createElement("div");
    row.className = "tag-row";
    const badgeWrapper = buildTagBadges([tag]);
    ...
    editButton.addEventListener("click", () => {
      issueTagId.value = tag.id;
      issueTagName.value = tag.name;
      issueTagColor.value = tag.color || DEFAULT_AREA_COLOR;
      issueTagSubmit.textContent = "Atualizar Tag";
    });
    ...
    deleteButton.addEventListener("click", async () => deleteIssueTag(tag));
    actions.appendChild(deleteButton);

    row.appendChild(actions);
    issueTagsList.appendChild(row);
  });
}

async function deleteIssueTag(tag) {
  if (!window.confirm(`Apagar a tag "${tag.name}"?`)) {
    return;
  }
  await api(`/api/issue-tags/${tag.id}`, { method: "DELETE" });
  resetIssueTagForm();
  await loadIssueReports();
  setStatus("Tag apagada.", "success");
}

function populateIssueFilters() {
  const currentRequester = issueRequesterFilter.value || "";
  const currentArea = issueAreaFilter.value || "";
  const currentTagIds = new Set(Array.from(issueTagFilter.selectedOptions).map((option) => option.value));
  const requesters = new Map();
  const areas = new Map();
  state.issueReports.forEach((issue) => {
    requesters.set(issue.requester_id, issue.requester_name || issue.requester_id);
    areas.set(issue.is_other_area ? "__other__" : issue.area_id, issueAreaLabel(issue));
  });

  issueRequesterFilter.innerHTML = '<option value="">Todos</option>';
  Array.from(requesters.entries())
    ...
    .forEach(([id, name]) => {
      ...
      issueRequesterFilter.appendChild(option);
    });

  issueAreaFilter.innerHTML = '<option value="">Todas</option>';
  Array.from(areas.entries())
    ...
    .forEach(([id, name]) => {
      ...
      issueAreaFilter.appendChild(option);
    });

  issueTagFilter.innerHTML = "";
  state.issueTags.forEach((tag) => {
    const option = document.createElement("option");
    option.value = tag.id;
    option.textContent = tag.name;
    option.selected = currentTagIds.has(tag.id);
    issueTagFilter.appendChild(option);
  });

  issueTitleSuggestions.innerHTML = "";
  state.issueReports.forEach((issue) => {
    const option = document.createElement("option");
    option.value = issue.title;
    issueTitleSuggestions.appendChild(option);
  });
}

function getFilteredIssueReports() {
  const titleSearch = normalizeText(issueTitleFilter.value);
  const requesterId = issueRequesterFilter.value;
  const areaId = issueAreaFilter.value;
  const selectedTagIds = new Set(Array.from(issueTagFilter.selectedOptions).map((option) => option.value));
  const dateFrom = issueDateFrom.value ? new Date(`${issueDateFrom.value}T00:00:00`) : null;
  const dateTo = issueDateTo.value ? new Date(`${issueDateTo.value}T23:59:59`) : null;

  const filtered = state.issueReports.filter((issue) => {
    const createdAt = issueCreatedDate(issue);
    const normalizedTitle = normalizeText(issue.title);
    const issueAreaId = issue.is_other_area ? "__other__" : issue.area_id;
    const issueTagIds = new Set((issue.tags || []).map((tag) => tag.id));
    return (!titleSearch || normalizedTitle.includes(titleSearch))
      && (!requesterId || issue.requester_id === requesterId)
      && (!areaId || issueAreaId === areaId)
      && (selectedTagIds.size === 0 || Array.from(selectedTagIds).some((tagId) => issueTagIds.has(tagId)))
      && (!dateFrom || createdAt >= dateFrom)
      && (!dateTo || createdAt <= dateTo);
  });

  const sortMode = issueSort.value || "executive";
  filtered.sort((a, b) => {
    if (a.status === "Concluído" && b.status !== "Concluído") return 1;
    if (a.status !== "Concluído" && b.status === "Concluído") return -1;
    if (sortMode === "requester") {
      return (b.requester_priority_score || 0) - (a.requester_priority_score || 0);
    }
    if (sortMode === "date") {
      return issueCreatedDate(b).getTime() - issueCreatedDate(a).getTime();
    }
    return (b.executive_priority_score || 0) - (a.executive_priority_score || 0)
      || (b.requester_priority_score || 0) - (a.requester_priority_score || 0)
      || issueCreatedDate(b).getTime() - issueCreatedDate(a).getTime();
  });
  return filtered;
}
...
function renderIssueReports() {
  ...
  const issues = getFilteredIssueReports();
  const thead = document.createElement("thead");
  thead.innerHTML = "<tr><th>Titulo</th><th>Solicitante</th><th>Area</th><th>Tags</th><th>Prior. Solicitante</th><th>Prior. Executivo</th><th>Status</th><th>Data</th><th>Acoes</th></tr>";
  ...
  issues.forEach((issue) => {
    ...
    tr.innerHTML = `
      ...
      <td>${escapeHtml(issue.requester_name || issue.requester_id)}</td>
      <td>${escapeHtml(issueAreaLabel(issue))}</td>
      <td></td>
      <td>${issue.requester_priority_score}</td>
      <td></td>
      <td></td>
      <td>${issueCreatedDate(issue).toLocaleDateString("pt-BR")}</td>
      <td></td>
    `;

    const tagsCell = tr.children[3];
    const executiveCell = tr.children[5];
    const statusCell = tr.children[6];
    if (isExecutive) {
      tagsCell.appendChild(buildIssueTagPicker(issue));
      executiveCell.appendChild(buildExecutivePriorityCell(issue));
      statusCell.appendChild(buildIssueStatusSelect(issue));
    } else {
      tagsCell.appendChild(buildTagBadges(issue.tags || []));
      executiveCell.textContent = issue.executive_priority_score || "-";
    ...
    detailButton.addEventListener("click", () => showIssueDetail(issue));
    ...
    if (isExecutive) {
      ...
      deleteButton.className = "danger";
      deleteButton.textContent = "Apagar";
      deleteButton.addEventListener("click", async () => deleteIssueReport(issue));
      actionsWrapper.appendChild(deleteButton);
    }
    tr.children[8].appendChild(actionsWrapper);
    tbody.appendChild(tr);
  });

  if (issues.length === 0) {
    const tr = document.createElement("tr");
    tr.innerHTML = '<td colspan="9" class="muted">Nenhum Issue Report encontrado.</td>';
    tbody.appendChild(tr);
  }
...
function buildIssueTagPicker(issue) {
  const wrapper = document.createElement("div");
  wrapper.className = "tag-picker";
  const selectedIds = new Set((issue.tags || []).map((tag) => tag.id));

  const chips = document.createElement("div");
  chips.className = "tag-picker-chips";
  ...
  suggestions.className = "tag-picker-suggestions hidden";
  wrapper.appendChild(suggestions);

  const saveTags = async () => {
    try {
      const tagIds = Array.from(selectedIds);
      await api(`/api/issue-reports/${issue.id}/tags`, {
        method: "PATCH",
        body: JSON.stringify({ tag_ids: tagIds }),
      });
      await loadIssueReports();
      setStatus("Tags do Issue atualizadas.", "success");
    } catch (error) {
      setStatus(error.message, "error");
    }
  };
  ...
  const renderChips = () => {
    chips.innerHTML = "";
    const selectedTags = state.issueTags.filter((tag) => selectedIds.has(tag.id));
  ...
  const renderSuggestions = () => {
    ...
    const availableTags = state.issueTags
      .filter((tag) => !selectedIds.has(tag.id))
      .filter((tag) => !query || normalizeText(tag.name).includes(query))
      .slice(0, 6);
  ...
  input.addEventListener("keydown", async (event) => {
    ...
    const firstOption = suggestions.querySelector(".tag-picker-option");
    if (firstOption) {
      firstOption.click();
    }
  });
...
async function deleteIssueReport(issue) {
  const confirmed = window.confirm(`Apagar o Issue "${issue.title}" da visualizacao?`);
  if (!confirmed) {
    return;
  }
  await api(`/api/issue-reports/${issue.id}`, { method: "DELETE" });
  await loadIssueReports();
  setStatus("Issue Report apagado da visualizacao.", "success");
}

function buildExecutiveGutEditor(issue) {
  return buildExecutivePriorityCell(issue);
}

function buildExecutivePriorityCell(issue) {
  const button = document.createElement("button");
  button.type = "button";
  button.className = "priority-cell-button";
  button.textContent = issue.executive_priority_score || "Priorizar";
  button.title = "Clique para preencher a prioridade executiva";
  button.addEventListener("click", () => openExecutivePriorityModal(issue));
  return button;
}

function buildIssueStatusSelect(issue) {
  ...
  select.addEventListener("change", async () => {
    ...
    try {
      await api(`/api/issue-reports/${issue.id}/executive-review`, {
        method: "PATCH",
        body: JSON.stringify({ status: select.value }),
      });
      await loadIssueReports();
      setStatus("Status do Issue Report atualizado.", "success");
    } catch (error) {
...
function calculateExecutivePriorityScore() {
  ...
  const tendency = Number(issueExecutiveTendency.value);
...
function updateExecutivePriorityScorePreview() {
  const score = calculateExecutivePriorityScore();
  issueExecutiveScore.textContent = score === null ? "-" : String(score);
}

function openExecutivePriorityModal(issue) {
  issuePriorityTitle.textContent = `Prioridade Executiva - ${issue.title}`;
  issuePriorityId.value = issue.id;
  issueExecutiveGravity.value = issue.executive_gravity || "";
  issueExecutiveUrgency.value = issue.executive_urgency || "";
  issueExecutiveTendency.value = issue.executive_tendency || "";
  updateExecutivePriorityScorePreview();
  issuePriorityPanel.classList.remove("hidden");
}

function showIssueDetail(issue) {
  issueDetailTitle.textContent = issue.title;
  issueDetailBody.innerHTML = `
    <section><strong>Ocorrencia</strong><p>${escapeHtml(issue.ocorrencia)}</p></section>
    <section><strong>Identificacao da causa</strong><p>${escapeHtml(issue.identificacao_causa)}</p></section>
...
function renderIndicators() {
  const isExecutive = state.user && state.user.role === "executivo";
  const isAreaManager = state.user && state.user.role === "gestor_area";
  ...
  indicatorHeader.className = "sticky-col sticky-col-header sticky-col-indicator";
  headerRow.appendChild(indicatorHeader);

  const areaHeader = document.createElement("th");
  areaHeader.textContent = "Area";
  areaHeader.className = "sticky-col sticky-col-header sticky-col-area";
  headerRow.appendChild(areaHeader);

  const maturityHeader = document.createElement("th");
  ...
  visibleIndicators.forEach((row) => {
    ...
    indicatorCell.className = "sticky-col sticky-col-indicator";
    if (areaTint) {
      indicatorCell.style.backgroundColor = areaTint;
    }
    if (isExecutive) {
    ...
    areaCell.className = "sticky-col sticky-col-area";
    if (areaTint) {
      areaCell.style.backgroundColor = areaTint;
    }
    areaCell.textContent = row.area_name || row.area_id;
    tr.appendChild(areaCell);

    const maturityCell = document.createElement("td");
    maturityCell.className = "sticky-col sticky-col-maturity";
    if (areaTint) {
...
async function showWeeklyPanel(row, month) {
  ...
  const monthItem = row.months.find((item) => item.month === month);
  const isNotApplicable = !!(monthItem && monthItem.not_applicable);
  ...
  naLabel.className = "checkbox-line na-toggle";
  const naCheckbox = document.createElement("input");
  naCheckbox.type = "checkbox";
  naCheckbox.checked = isNotApplicable;
  naLabel.appendChild(naCheckbox);
  naLabel.appendChild(document.createTextNode(" Indicador nao calculado neste mes"));
  weeklyForm.appendChild(naLabel);
  ...
  saveButton.disabled = isNotApplicable;
  ...
  naCheckbox.addEventListener("change", async () => {
    try {
      await api(`/api/indicators/${row.indicator_id}/monthly-na`, {
        method: "POST",
        body: JSON.stringify({
          year: state.year,
          month,
          is_not_applicable: naCheckbox.checked,
        }),
      });
      await loadIndicators();
      const refreshedRow = state.indicators.find(
        (indicator) => indicator.indicator_id === row.indicator_id,
      );
      if (refreshedRow) {
        await showWeeklyPanel(refreshedRow, month);
      }
      setStatus(
        naCheckbox.checked
          ? "Mes marcado como N/A."
          : "Mes voltou a calcular normalmente.",
        "success",
      );
    } catch (error) {
      setStatus(error.message, "error");
...
async function saveWeeklyValues() {
  const inputs = Array.from(weeklyForm.querySelectorAll('input[name^="week-"]'));
  let sentCount = 0;
...
async function openCreateIndicatorPanel(indicatorRow = null) {
  ...
  if (indicatorRow) {
    ...
    ) ? "" : String(indicatorRow.maturity_level);
...
async function submitIssueReport(event) {
  event.preventDefault();
  const areaValue = issueAreaSelect.value;
  const payload = {
    title: document.getElementById("issue-title").value,
    area_id: areaValue === "__other__" ? null : areaValue,
    is_other_area: areaValue === "__other__",
    requester_gravity: Number(document.getElementById("issue-requester-gravity").value),
    requester_urgency: Number(document.getElementById("issue-requester-urgency").value),
    requester_tendency: Number(document.getElementById("issue-requester-tendency").value),
    ocorrencia: document.getElementById("issue-ocorrencia").value,
    identificacao_causa: document.getElementById("issue-identificacao-causa").value,
    proposta_solucao: document.getElementById("issue-proposta-solucao").value,
  };
...
tabIndicators.addEventListener("click", async () => {
  try {
    await showAppTab("indicators");
  } catch (error) {
    setStatus(error.message, "error");
  }
});
...
[issueDateFrom, issueDateTo, issueRequesterFilter, issueAreaFilter, issueTagFilter, issueTitleFilter, issueSort]
  .forEach((element) => {
...
manageTagsBtn.addEventListener("click", () => {
  issueTagsPanel.classList.toggle("hidden");
});

document.getElementById("issue-tag-clear").addEventListener("click", resetIssueTagForm);

issueTagForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  try {
    const tagId = issueTagId.value;
    const payload = {
      name: issueTagName.value,
      color: issueTagColor.value || null,
    };
    if (tagId) {
      await api(`/api/issue-tags/${tagId}`, {
        method: "PUT",
        body: JSON.stringify(payload),
      });
      setStatus("Tag atualizada.", "success");
    } else {
      await api("/api/issue-tags", {
        method: "POST",
        body: JSON.stringify(payload),
      });
      setStatus("Tag criada.", "success");
    }
    resetIssueTagForm();
    await loadIssueReports();
  } catch (error) {
...
document.getElementById("issue-detail-close").addEventListener("click", () => {
  issueDetailPanel.classList.add("hidden");
});

[issueExecutiveGravity, issueExecutiveUrgency, issueExecutiveTendency].forEach((element) => {
  element.addEventListener("change", updateExecutivePriorityScorePreview);
  element.addEventListener("input", updateExecutivePriorityScorePreview);
});

document.getElementById("issue-priority-cancel").addEventListener("click", () => {
  issuePriorityPanel.classList.add("hidden");
  issuePriorityForm.reset();
});

issuePriorityForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  try {
    const issueId = issuePriorityId.value;
    await api(`/api/issue-reports/${issueId}/executive-review`, {
      method: "PATCH",
      body: JSON.stringify({
        executive_gravity: Number(issueExecutiveGravity.value),
        executive_urgency: Number(issueExecutiveUrgency.value),
        executive_tendency: Number(issueExecutiveTendency.value),
      }),
    });
    issuePriorityPanel.classList.add("hidden");
    issuePriorityForm.reset();
    await loadIssueReports();
    setStatus("Prioridade executiva atualizada.", "success");
  } catch (error) {
    setStatus(error.message, "error");
  }
});
...
monthlyPlanningForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (mpProjectedGroup.classList.contains("hidden") === false) {
      await api(`/api/indicators/${indicator_id}/monthly-projection`, {
        method: "POST",
        body: JSON.stringify({
          year,
          month,
          projected_value: mpProjectedValue.value.trim() || null,
        }),
      });
      changed = true;
    }

    if (mpTargetGroup.classList.contains("hidden") === false) {
      await api(`/api/indicators/${indicator_id}/monthly-target`, {
        method: "POST",
        body: JSON.stringify({
          year,
          month,
          target_value: mpTargetValue.value.trim() || null,
        }),
      });
      changed = true;
    }
