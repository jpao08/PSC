...
class SupabaseIndicatorRepository(IndicatorRepositoryPort):
    ...
    def create_indicator(self, indicator: NewIndicator) -> Indicator:
        ...
        payload = {
            ...
            "maturity_level": str(indicator.maturity_level) if indicator.maturity_level is not None else None,
    ...
    def delete_indicator_with_history(self, indicator_id: str) -> None:
        ...
        self.client.table("indicator_month_not_applicable").delete().eq("indicator_id", indicator_id).execute()
    ...
    def list_month_not_applicable(
        ...
        return [
            IndicatorMonthNotApplicable(
                ...
                year=int(row["year"]),
                month=int(row["month"]),
                marked_by=str(row["marked_by"]) if row.get("marked_by") else None,
            )
            for row in rows
        ]
    ...
    def _to_indicator(
        ...
        return Indicator(
            ...
            is_active=bool(row.get("is_active", True)),
            created_by=str(row["created_by"]) if row.get("created_by") else None,
            maturity_level=_to_decimal(row.get("maturity_level")),
        )
...
class SupabaseIssueReportRepository(IssueReportRepositoryPort):
    def __init__(self, client: Client) -> None:
        self.client = client
    ...
    def create_issue_report(self, issue: NewIssueReport) -> IssueReport:
        payload = {
            ...
            "problem_description": issue.ocorrencia,
            "observed_impact": issue.identificacao_causa,
            "attempted_solution": "-",
            "requested_action": issue.proposta_solucao,
        }
        response = self.client.table("issue_reports").insert(payload).execute()
        return self._to_issue_report((response.data or [])[0])

    def list_issue_reports(self, requester_id: str | None = None) -> list[IssueReport]:
        query = self.client.table("issue_reports").select("*").eq("is_deleted", False)
        if requester_id:
        ...
        tag_map = self._list_issue_tags_map([str(row["id"]) for row in rows if row.get("id")])
        return [
            self._to_issue_report(
                row,
    ...
    def update_executive_review(
        ...
        if executive_gravity is not None:
            payload["executive_gravity"] = executive_gravity
        if executive_urgency is not None:
            payload["executive_urgency"] = executive_urgency
        if executive_tendency is not None:
            payload["executive_tendency"] = executive_tendency
        if status is not None:
            payload["status"] = status

        response = self.client.table("issue_reports").update(payload).eq("id", issue_id).execute()
        rows = response.data or []
        if not rows:
            raise NotFoundError("Issue Report nao encontrado para atualizacao.")
        return self._get_issue_report_by_id(issue_id)
    ...
    def soft_delete_issue_report(self, issue_id: str, deleted_by: str) -> None:
        ...
        if not (response.data or []):
            raise NotFoundError("Issue Report nao encontrado para exclusao.")

    def list_issue_tags(self) -> list[IssueTag]:
        response = (
            self.client.table("issue_tags")
            .select("*")
            .eq("is_active", True)
            .order("name")
            .execute()
        )
        return [self._to_issue_tag(row) for row in response.data or []]
    ...
    def create_issue_tag(self, name: str, color: str | None, created_by: str) -> IssueTag:
        ...
        response = self.client.table("issue_tags").insert(payload).execute()
        return self._to_issue_tag((response.data or [])[0])
    ...
    def update_issue_tag(self, tag_id: str, name: str, color: str | None) -> IssueTag:
        ...
        rows = response.data or []
        if not rows:
            raise NotFoundError("Tag nao encontrada para atualizacao.")
        return self._to_issue_tag(rows[0])
    ...
    def deactivate_issue_tag(self, tag_id: str) -> None:
        ...
        if not (response.data or []):
            raise NotFoundError("Tag nao encontrada para exclusao.")

    def replace_issue_tags(self, issue_id: str, tag_ids: list[str], updated_by: str) -> IssueReport:
        response = self.client.table("issue_reports").select("id").eq("id", issue_id).limit(1).execute()
        if not (response.data or []):
            raise NotFoundError("Issue Report nao encontrado para tags.")

        self.client.table("issue_report_tags").delete().eq("issue_id", issue_id).execute()
        rows = [
            {
                "issue_id": issue_id,
                "tag_id": tag_id,
        ...
        if rows:
            self.client.table("issue_report_tags").insert(rows).execute()
        return self._get_issue_report_by_id(issue_id)
    ...
    def _list_issue_tags_map(self, issue_ids: list[str]) -> dict[str, list[IssueTag]]:
        if not issue_ids:
            return {}
        response = (
            self.client.table("issue_report_tags")
            .select("issue_id, issue_tags(id,name,color,is_active)")
            .in_("issue_id", issue_ids)
            .execute()
        )
        tag_map: dict[str, list[IssueTag]] = {}
        for row in response.data or []:
            issue_id = str(row["issue_id"])
            tag_row = row.get("issue_tags") or {}
            if not tag_row or not tag_row.get("is_active", True):
                continue
            tag_map.setdefault(issue_id, []).append(self._to_issue_tag(tag_row))
        for tags in tag_map.values():
            tags.sort(key=lambda tag: tag.name.strip().lower())
        return tag_map

    def _get_issue_report_by_id(self, issue_id: str) -> IssueReport:
        response = self.client.table("issue_reports").select("*").eq("id", issue_id).limit(1).execute()
        rows = response.data or []
        if not rows:
            raise NotFoundError("Issue Report nao encontrado.")
        row = rows[0]
        ...
        tags = self._list_issue_tags_map([issue_id]).get(issue_id, [])
        return self._to_issue_report(
            row,
    ...
    @staticmethod
    def _to_issue_tag(row: dict[str, Any]) -> IssueTag:
        return IssueTag(
            id=str(row["id"]),
            name=str(row["name"]),
            color=str(row["color"]) if row.get("color") else None,
            is_active=bool(row.get("is_active", True)),
        )

    @staticmethod
    def _to_issue_report(
        row: dict[str, Any],
        ...
        return IssueReport(
            ...
            requester_gravity=int(row["requester_gravity"]),
            requester_urgency=int(row["requester_urgency"]),
            requester_tendency=int(row["requester_tendency"]),
            requester_priority_score=int(row.get("requester_priority_score") or 0),
            executive_gravity=int(row["executive_gravity"]) if row.get("executive_gravity") else None,
            executive_urgency=int(row["executive_urgency"]) if row.get("executive_urgency") else None,
            executive_tendency=int(row["executive_tendency"]) if row.get("executive_tendency") else None,
            executive_priority_score=(
                int(row["executive_priority_score"])
                if row.get("executive_priority_score") is not None
                else None
            ),
            ocorrencia=str(row.get("ocorrencia") or row.get("problem_description") or ""),
            identificacao_causa=str(
                row.get("identificacao_causa") or row.get("observed_impact") or ""
            ),
            proposta_solucao=str(row.get("proposta_solucao") or row.get("requested_action") or ""),
            status=str(row.get("status") or "Não Iniciada"),
